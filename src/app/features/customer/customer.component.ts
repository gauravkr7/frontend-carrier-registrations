...
loadCompany() {
  this.serviceAuthService.getAllCompanies().subscribe((data: any) => {
    // Only include companies with status 'client'
    this.companies = data.filter((company: any) => company.status?.toLowerCase() === 'client');
...
searchCompany() {
  this.filteredCompanies = this.companies.filter(company =>
    (company.dot && company.dot.includes(this.searchQuery)) ||
    (company.companyName && company.companyName.toLowerCase().includes(this.searchQuery.toLowerCase())) ||
    (company.mc && company.mc.includes(this.searchQuery))
  );
  // Ensure only 'client' status is shown
  this.filteredCompanies = this.filteredCompanies.filter(company => company.status?.toLowerCase() === 'client');
  this.calculateTotalPages();
  this.updatePaginatedCompanies();
  this.cdr.detectChanges();
}
...
searchDot() {
  console.log("Searching DOT:", this.newCompany.dot);
  // Add API call or logic here
}
onAddCompanyClick() {
  console.log("Add Company clicked");
  // Reset form / prepare data here
}
